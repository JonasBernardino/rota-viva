<form class="municipality-selector" action="/municipios/selecionar" method="get">
    ...
    <select id="municipality-selector" name="municipality" aria-label="Selecionar município" data-municipality-selector>
        <option value="">Selecione o município</option>
        @foreach ($municipalityOptions ?? [] as $municipality)
            <option value="{{ $municipality->slug }}" data-url="{{ $municipality->public_url }}" @selected(($currentTenant?->slug ?? null) === $municipality->slug)>
                {{ $municipality->nome }}
            </option>
        @endforeach
    </select>
